...
const { getTodayDate, processTableRow, findAggregateData } = require("./utils");
...
const scrapeData = async () => {
  ...
  const stateWiseData = await page.evaluate(() => {
    const tableRows = document.querySelectorAll("table.statetable tbody tr");
    const statesData = tableRows.map((tableRow) => processTableRow(tableRow));
    return statesData;
  });

  await browser.close();

  const aggregateData = findAggregateData(stateWiseData);
...
//         if (index === 1) state[titles[index]] = col.textContent;
//         else if (index === 3 || index === 5 || index === 7) {
//           let data = parseInt(col.textContent) || 0;

//           if (col.querySelector(".down") !== null) data *= -1;
